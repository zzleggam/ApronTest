...
Shader "InfiniGRASS/Vertex Grass Spec UnityGrass" {
    Properties {
...
void FastSinCos (float4 val, out float4 s, out float4 c) {
	...
	c = 1 + r5 * cos8.x + r6 * cos8.y + r7 * cos8.z + r8 * cos8.w;
           ...
           //     v.vertex.xyz += (normalize((float3(1,0.5,0.5)+v.normal))*node_389.r*sin(((node_389.b*3.141592654)+node_392.g+node_392.b))*0.16);
				...
				////////// UNITY GRASS
				float4 _WaveAndDistance =  float4(0.01, 0.01, 0.1, 5);	// wind speed, wave size, wind amount, max sqr distance
				float waveAmount = v.vertex.y;
				...
				float4 waves;
				waves = v.vertex.x * _waveXSize* _Time.y *4;
