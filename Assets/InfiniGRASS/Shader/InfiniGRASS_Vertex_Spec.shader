...
Shader "InfiniGRASS/Vertex Grass Spec" {
    Properties {
        ...
        Pass {
           ...
           //     v.vertex.xyz += (normalize((float3(1,0.5,0.5)+v.normal))*node_389.r*sin(((node_389.b*3.141592654)+node_392.g+node_392.b))*0.16);
                ...
                if( o.uv0.y > 0.1){
                	...
                	//v.vertex.xyz += 0.02*sin(_TimeControl1.w*_Time.y +o.posWorld.x)+0.02*cos( _TimeControl1.w*_Time.y +o.posWorld.z);
                	//v.vertex.xyz += 0.01*sin(_TimeControl1.w*_Time.y +55)+0.01*cos( _TimeControl1.w*_Time.y*4 +o.posWorld.z);
                	//v.vertex.x += cos(_Time.y*4 +o.posWorld.x)+sin(_Time.y*4  +o.posWorld.z);
                	//v.vertex.z += cos(_Time.y*4 +o.posWorld.x)+sin(_Time.y*4  +o.posWorld.z);
                	//v.vertex.x += sin(_Time.y*1 +o.posWorld.x)+cos(_Time.y*2  +o.posWorld.z +111);
                	//v.vertex.x += 2;
				}
				if( o.uv0.y > 0.6){
					v.vertex.x += 2 / scaleY;
...
/////// Normals:
        ...
        //        clip(node_1.a - _Cutoff);
              ...
              //  lightDirection = reflect(lightDirection,normalDirection);
...
////// Specular:
            ...
            //    v.vertex.xyz += 0.5*sin(_TimeControl1.w*_Time.y +o.posWorld.x)+0.5*cos( _TimeControl1.w*_Time.y +o.posWorld.z);
              //  finalColor += dot(halfDirection,_WorldSpaceLightPos0.xyz)*(0.6*sin(_TimeControl1.w*_Time.y +i.posWorld.x +i.posWorld.z)+cos( _TimeControl1.w*_Time.y +i.posWorld.z))+0.1;
