...
Shader "InfiniGRASS/InfiniGrass Circular Wind" {
    Properties {
...
//
               ...
               //      if( distance(_InteractPos.z,o.posWorld.z) < _StopMotionThreshold/2){                  
                	...
                	//SpeedFac  =  10*(o.posWorld-_InteractPos ) *_WaveControl1.w * cos(_Time.y)+1.1;
                	SpeedFac  =  3*(o.posWorld-_InteractPos) *_WaveControl1.w + _InteractAmpFreqRad.z*(1.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos) - 2.71*cross(float3(0,1,0),_InteractPos-o.posWorld))  + _InteractAmpFreqRad.x*(o.posWorld-_InteractPos) *_WaveControl1.w *sin(o.posWorld.z+_InteractAmpFreqRad.y*_Time.y);
                	_BulgeScale = _BulgeScale * (1/(distA+0.01));
                	//_OceanCenter.z = _OceanCenter.z * (1/(distA*distA+0.01));
...
//						}
               ...
               /////////////////////////////////////////////////////////////////////////////////////////// END LOCAL INTERACTOR 1 /////////////////////////////////////////////

               	/////////////////////////////////////////////////////////////////////////////////////////// LOCAL INTERACTOR 2 /////////////////////////////////////////////
  				SpeedFac = float3(0,0,0);   
                distA =  distance(_InteractPos1,o.posWorld)/ (_StopMotionThreshold*1);      
                if( distance(_InteractPos1,o.posWorld) < _StopMotionThreshold*1){               
                	SpeedFac  =  3*(o.posWorld-_InteractPos1) *_WaveControl1.w + _InteractAmpFreqRad.z*(1.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos1) - 2.71*cross(float3(0,1,0),_InteractPos1-o.posWorld))  + _InteractAmpFreqRad.x*(o.posWorld-_InteractPos1) *_WaveControl1.w *sin(o.posWorld.z+_InteractAmpFreqRad.y*_Time.y);
                	_BulgeScale = _BulgeScale * (1/(distA+0.01));
...
/// Final Color:
        ...
        Pass {
           ...
           // #pragma exclude_renderers gles xbox360 ps3 flash 
            ...
            float3 _InteractPos;float3 _InteractPos1;
            float _FadeThreshold;
            float _StopMotionThreshold;
            float _SmoothMotionFactor;
            float _WaveXFactor;
           	float _WaveYFactor;
           	 ...
           	 float3 _InteractSpeed;	 float3 _InteractAmpFreqRad;
...
float3 SpeedFac = float3(0,0,0);  //	SpeedFac =  _InteractSpeed;  
               ...
               //      if( distance(_InteractPos.z,o.posWorld.z) < _StopMotionThreshold/2){                  
                	...
                	//SpeedFac =  _InteractSpeed * _WaveControl1.w;
                	//SpeedFac  =  10*(o.posWorld-_InteractPos ) *_WaveControl1.w * cos(_Time.y)+1.1;
                	//SpeedFac  =  8*(o.posWorld-_InteractPos) *_WaveControl1.w * (cos(_Time.y)-0.4) - 1.1*cross(float3(0,1,0),o.posWorld-_InteractPos) + 2.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos);
                	SpeedFac  =  3*(o.posWorld-_InteractPos) *_WaveControl1.w + _InteractAmpFreqRad.z*(1.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos) - 2.71*cross(float3(0,1,0),_InteractPos-o.posWorld))  + _InteractAmpFreqRad.x*(o.posWorld-_InteractPos) *_WaveControl1.w *sin(o.posWorld.z+_InteractAmpFreqRad.y*_Time.y);
                	_BulgeScale = _BulgeScale * (1/(distA+0.01)); //turbulence
                	//_OceanCenter.z = _OceanCenter.z * (1/(distA*distA+0.01));
...
float3 SpeedFac = float3(0,0,0);  //	SpeedFac =  _InteractSpeed;  
               ...
               //      if( distance(_InteractPos.z,o.posWorld.z) < _StopMotionThreshold/2){                  
                	...
                	//SpeedFac =  _InteractSpeed * _WaveControl1.w;
                	//SpeedFac  =  10*(o.posWorld-_InteractPos) *_WaveControl1.w * (cos(_Time.y)-0.4) - 1.1*cross(float3(0,1,0),o.posWorld-_InteractPos) + 2.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos);
                	//SpeedFac  =  8*(o.posWorld-_InteractPos) *_WaveControl1.w * (cos(_Time.y)-0.4) - 1.1*cross(float3(0,1,0),o.posWorld-_InteractPos) + 2.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos);
                	//SpeedFac  =  2*(o.posWorld-_InteractPos) *_WaveControl1.w * (cos(_Time.y)-1.8) - 2.71*cross(float3(0,1,0),_InteractPos-o.posWorld)* (cos(_Time.y*11.2)+1.4) + 1.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos);
                	//SpeedFac  =  5*(o.posWorld-_InteractPos) *_WaveControl1.w * (cos(_Time.y*5)-0.8) - 2.71*cross(float3(0,1,0),_InteractPos-o.posWorld)* 1 + 1.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos);
                	//SpeedFac  =  5*(o.posWorld-_InteractPos) *_WaveControl1.w * 1 - 2.71*cross(float3(0,1,0),_InteractPos-o.posWorld)* 1 + 1.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos);
                	SpeedFac  =  3*(o.posWorld-_InteractPos) *_WaveControl1.w + _InteractAmpFreqRad.z*(1.1*cross(float3(0,1,0.5),o.posWorld-_InteractPos) - 2.71*cross(float3(0,1,0),_InteractPos-o.posWorld))  + _InteractAmpFreqRad.x*(o.posWorld-_InteractPos) *_WaveControl1.w *sin(o.posWorld.z+_InteractAmpFreqRad.y*_Time.y);
                	_BulgeScale = _BulgeScale * (1/(distA+0.01));
                	//_OceanCenter.z = _OceanCenter.z * (1/(distA*distA+0.01));
