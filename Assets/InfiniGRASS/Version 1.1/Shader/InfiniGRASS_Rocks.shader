...
Shader "InfiniGRASS/Rocks" {
    Properties {
        ...
        _RippleScale ("Vary Height", Float ) = 0
           ...
           _TintFrequency("tint frequency", Float) = 0.01
        ...
        _SnowBlend ("Snow Blend", Range(-33, 51)) = 0.4     
        ...
        _Shininess ("Shininess", Range (0.01, 1)) = 0.078125
           _SnowCoverage ("_SnowCoverage", Range (0, 33.8)) = 3
    }
    SubShader {
        Tags {
            //"Queue"="Opaque"
        ...
        half _Shininess; 
            ...
            struct VertexInput {
               ...
               // float2 texcoord2a : TEXCOORD2;
               // float2 texcoord3a : TEXCOORD3;
               // float2 texcoord4a : TEXCOORD4;
            };
            struct VertexOutput {
                ...
                float4 vertexColor : COLOR;
                LIGHTING_COORDS(5,6)
               // float2 uvA_Bump : TEXCOORD7;
           		//float2 uvA_SnowTexture : TEXCOORD8;
            	//float2 uvA_SnowBump : TEXCOORD9;
                #ifndef LIGHTMAP_OFF
            ...
            VertexOutput vert (VertexInput v) {
                ...
                //SNOW
              //  o.uvA_Bump = v.texcoord0;
               // o.uvA_SnowTexture = v.texcoord0;
               // o.uvA_SnowBump = v.texcoord0;
...
////// Specular:
                ...
                float3 specular = 1 * pow(max(0,dot(halfDirection,normalDirection)),specPow) * specularColor	*2;
//                #ifndef LIGHTMAP_OFF
//                    #ifndef DIRLIGHTMAP_OFF
//                        specular *= lightmap;
//                    #else
...
specular *= ((attenuation) * _LightColor0.xyz);////////////// INFINIGRASS FIX !!!! - fixes flickering shadows - disable all above
             ...
             // 	specular = specular * (i.uv0.y*2-0.5) ;              
              ...
              finalColor = lerp(finalColor, finalColor*_Color,_TintPower*1*(0.9+0.6*cos(i.posWorld.x*2*_TintFrequency)+0.6*sin(i.posWorld.z*3*_TintFrequency)+0.6*sin(i.posWorld.z*1*_TintFrequency+0.1)));
...
//	_SnowCoverage=211.3;
				...
				float4 MainTexColor = node_1;
      //      o.Normal = UnpackNormal(tex2D(_Bump, IN.uv_Bump));           

       //     o.Alpha = MainTexColor.a;  
            float DirN = dot(normalLocal,_Direction)	; 
            
            
            
            float Check = lerp(1,-1,_SnowCoverage/7); 
            if(DirN >= Check && (i.uv0.y >= 1-(3 * _SnowCoverage) * col.r* col.r) )
            {      
	...
	//                col.rgb = float4(i.uv0.y,i.uv0.y,i.uv0.y,1)*1;   
	           	    ...
	           	    col.rgb = lerp (  MainTexColor.rgb *1 , SnowTexColor.rgb*_LightIntensity,pow((1-(Check/(DirN))),_SnowBlend/2)) ;                
                	//normalLocal = normalize(normalLocal + UnpackNormal(tex2D(_SnowBump, i.uvA_SnowBump))*_SnowBumpDepth);                           
	            }
	            else
	            {
					//col = col * input.color * input.color.a *_UnityTerrainTreeTintColorSM *1.5;						
